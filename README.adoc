...
For each key you would like to provide one or more aliases for, make a row in the array you pass to `aliases`, with a key corresponding to the key you'd like to alias to, and a value containing an array of string or a single string that correspond to the alias(es) you want that original key to have.
